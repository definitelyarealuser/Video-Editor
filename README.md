...
- **Render MP3**: exports a second file alongside the MP4 containing just the main clip's audio — the PNG-hold silence at the start and end is excluded, but the clip still fades up from silence and back down over the crossfade duration, so it sounds like a clean standalone edit rather than an abrupt cut. This applies regardless of the "apply crossfade to audio too" setting above (which only affects the audio inside the full MP4). The checkbox lives with the local-save settings and controls whether you get an MP3 of your own; publishing to SoundCloud renders one regardless, since it has to have something to upload. Unticking it does not turn SoundCloud publishing off - it used to, silently.
