...
5. Optionally check **Normalize audio** and pick a target loudness (-20 to -10 LUFS, defaults to -14) and/or **Also render an MP3**. Optionally fill in **Also save the MP4/MP3 to this folder on this computer** - full local folder paths (e.g. `C:\Users\andyw\Videos\Sermons` or `/Users/andy/Movies/Sermons`); leave either blank to skip. Click **Browse…** next to either field to pick the folder from a simple in-app browser instead of typing the path by hand - it starts at whatever's already in the field, or your home folder if it's empty; click a folder name to go into it, **.. (up one level)** to back out, then **Select This Folder**. This only makes sense because the server runs on your own machine - it copies the finished file(s) there directly, in addition to the in-browser download buttons, so you don't have to manually move them out of your Downloads folder every time. Both are remembered for next time, same as the other render settings.
