...
3. Optionally, in that same panel, build the list of showcases (albums) every published video should get added to automatically: paste a showcase's link (or just its ID) into the box under **Showcases** and click **Add**. Each showcase appears as a row with its real name, and **Remove** takes one out. Every add and remove saves on the spot - there's no separate Save step for this list - and the list can be left empty, or changed any time via the same panel (now labeled **Vimeo settings…**).
...
With Vimeo connected, clicking **Render Video** shows a **"Publish to Vimeo?"** dialog - it's a deliberate choice every time, never remembered from a previous render. The title and description aren't re-entered here - they're the file name (Series / Sermon Title / Speaker's Name / Sermon Date) and Core Text you already filled in on the main form. Privacy defaults to Public (adjustable per-render via the dropdown). The dialog just checks every configured showcase by default; uncheck any you don't want this particular video added to. Choose **Render & Publish to Vimeo**, and once rendering finishes, the app uploads the MP4 with that title and description, sets the rectangular bookend graphic as the video's thumbnail, and adds it to whichever showcases were checked - no separate manual step. A results panel shows the Vimeo link, whether the custom thumbnail took, and which showcases succeeded or failed (one showcase failing, e.g. a stale ID, doesn't block the others - a thumbnail failure is reported the same way and likewise doesn't block the rest). The thumbnail line is shown on success as well as failure: after setting the picture the app asks Vimeo what the video's thumbnail actually is (`pictures.type` reads `custom` for an uploaded image, `default` for a frame Vimeo picked itself) and says which it got, retrying briefly since the video is usually still transcoding. A silent success and a thumbnail that quietly didn't take used to look identical. Choosing **Render Only** just renders normally, same as if Vimeo weren't configured at all. **Cancel** (or clicking outside the dialog, or pressing Escape) backs out entirely - no render happens at all, so you can adjust settings first and click **Render Video** again.
...
3. Optionally, in that same panel, build the list of playlists every published track should get added to automatically: paste a playlist's ordinary web link into the box under **Playlists** and click **Add** - the app looks up its ID and name for you (this only works once connected). Each playlist appears as a row with its real name, and **Remove** takes one out. Every add and remove saves on the spot - there's no separate Save step for this list - and the list can be left empty, or changed any time via the same panel (now labeled **SoundCloud settings…**).

That's the whole setup, and it only needs doing once per machine - reopen the panel any time to change the playlists, or click **Disconnect SoundCloud** to sign out (keeping the saved Client ID/Secret) or **Forget these credentials** to clear everything and start over. (Advanced/scripted installs can instead set `SOUNDCLOUD_CLIENT_ID`/`SOUNDCLOUD_CLIENT_SECRET` as environment variables in `.env` - see `.env.example` - which take priority over the in-app panel and lock those two fields; this isn't needed for normal use. `SOUNDCLOUD_PLAYLIST_IDS`/`VIMEO_SHOWCASE_IDS` behave differently: they only seed the list for an install that has never touched the in-app controls, and the app takes over permanently the first time a playlist or showcase is added or removed there. Managing those lists is routine work, so the app always wins.)
...
- **Normalize audio**: runs the sermon's own audio through `loudnorm` (EBU R128) targeting the chosen integrated loudness, before the silent bookend holds are attached. Single-pass, so expect the result to land within roughly half a LU of the target. (It used to normalize the combined audio *after* the bookend silence was added, which left the video under-corrected - the quieter the week, the worse it got, while the MP3 landed on target. Same sermon, two different levels. Fixed.)
- **Check audio level**: measures the clip and says whether normalizing is worth turning on, so "does this week sound quieter than last week?" has an answer instead of being judged by ear each Sunday. It runs automatically as soon as a video finishes uploading, and the button re-runs it against the current trim - useful after trimming off a loud pre-roll. It reports the integrated loudness (LUFS), true peak (dBTP) and how much gain normalizing would apply, then one of: levels look good, normalizing optional, or normalizing recommended. Recommended means the level is more than 3 LU off target, or peaks are above -1 dBTP (which risks distortion once Vimeo and SoundCloud re-encode it). It only advises - nothing is switched on unless you click **Turn on normalizing** or tick the box yourself.
  - It lives at the end of the form, after the save-path settings, because that is where the decision is made - but nothing has to be clicked to get it. It measures on its own as soon as the video finishes uploading, and re-measures whenever the trim changes by any route (dragging a handle, the nudge buttons, Re-Edit), so the answer is already waiting by the time anyone scrolls to it and always describes the range actually being published rather than the raw upload. **Re-check** is only there for when you want to force it. A trim landing mid-measurement cancels the run it invalidated - the browser drops the request and the server kills the ffmpeg behind it - rather than queueing the new measurement behind a result already known to be stale, which on a 45-minute service would be another ten seconds of waiting for an answer nobody would read.
  - Speed: clips up to an hour are measured straight through, which is exact. Longer ones are sampled at up to 120 short windows spread evenly across the trimmed range, seeked to directly and concatenated into one pass, so the answer arrives in seconds whatever the length. The line under the recommendation says which of the two happened and how long it took. Measured on a 4-core machine, a 45-minute service takes ~10s exact (it was 35s before the analysis moved off `loudnorm` onto `ebur128`, which reports the same figures about three times faster).
  - Why many short windows and not one long chunk, on the sampled path: coverage is what catches a one-off loud moment. Sampling a 45-minute test clip with 12 x 20s windows walked straight past a 60-second burst and under-read its peak by 17 dB; ~90 short windows caught it to within 0.3 dB, for about the same total audio. A sampled peak is still a lower bound - if it says peaks are too hot they certainly are, but a brief spike between windows can be missed. Anything an hour or under avoids that entirely by being measured in full.
- **Render MP3**: exports a second file alongside the MP4 containing just the main clip's audio — the PNG-hold silence at the start and end is excluded, but the clip still fades up from silence and back down over the crossfade duration, so it sounds like a clean standalone edit rather than an abrupt cut. This applies regardless of the "apply crossfade to audio too" setting above (which only affects the audio inside the full MP4). An MP3 is made on every render - there is no setting for it. To skip it, use **Skip the MP3** in the SoundCloud step, which renders the video on its own. That step is the natural home for the choice because skipping the MP3 and not publishing are the same decision: there is nothing to upload without a file. If SoundCloud isn't connected that step never appears, and an MP3 is always made.

## Output quality

Fixed, with nothing to choose: H.264 at CRF 18 (`libx264`) for video, 192 kbps for MP3. There used
to be an **Output quality** panel with video-quality and MP3-bitrate dropdowns, labelled with real
file-size estimates measured by test-encoding a sample of your footage on every trim change. It was
removed: Vimeo and SoundCloud re-encode whatever they are given, so the choice traded render time
