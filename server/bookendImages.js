...
/**
 * A small local library of bookend graphics (PNGs, usually) that have been used before, so a
 * repeat one can be picked from a list instead of uploaded again every time. Nothing here
 * leaves the machine - a JSON index plus the image files themselves, both gitignored under
 * data/, same as history.js.
 */

const DATA_DIR = path.join(__dirname, '..', 'data');
const IMAGES_DIR = path.join(DATA_DIR, 'bookend-images');
const INDEX_PATH = path.join(DATA_DIR, 'bookend-images.json');
 ...
 * graphic (which happens a lot, since a church tends to reuse a handful of bookend images)
 * doesn't pile up duplicate entries, it just bumps the existing one's lastUsedAt.
 */
...
module.exports = { IMAGES_DIR, saveImage, listImages, getImage, touchImage, deleteImage };
